...
export default async function InstantQuotePage({
  ...
  const requestedSlug = typeof service === "string" ? service : undefined;
  const initialService = requestedSlug ? getServiceBySlug(requestedSlug) : undefined;
    ...
    <>
      ...
      <section className="bg-ink-50/60 py-16 sm:py-20">
        <Container className="max-w-2xl text-center">
          ...
          <h1 className="mt-5 font-display text-balance text-4xl font-bold tracking-tight text-ink-950 sm:text-5xl">
            {initialService ? `Get your instant quote for ${initialService.shortName.toLowerCase()}` : "Get your instant cleaning quote"}
          </h1>
      ...
      <section className="py-16 sm:py-20">
        <Container className="max-w-4xl">
          <InstantQuoteWizard initialServiceSlug={initialService?.slug} />
        </Container>
      </section>
